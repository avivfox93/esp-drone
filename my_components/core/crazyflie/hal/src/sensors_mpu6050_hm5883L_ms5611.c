...
// Check if there is a new temp update
    bmp280_read_pressure_temperature(&rawPressure, &rawTemp);
    float altitude = 44330.0f * (1.0f - powf(rawPressure / 101325.0f, 0.1903f));
    sensorData.baro.pressure = (float)rawPressure;
    sensorData.baro.asl = altitude;
    sensorData.baro.temperature = (float)rawTemp / 10.0f;
...
static void sensorsDeviceInit(void)
{
    ...
    if(bmp280_init(&bmp280) == BSTDR_OK){
    	isBarometerPresent = true;
        bmp280_set_oversamp_temperature(BMP280_OVERSAMP_1X);
        bmp280_set_oversamp_pressure(BMP280_OVERSAMP_1X);
    }
#endif
